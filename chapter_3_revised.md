...
subtitle: "Revised draft, tightened"
...
There are several prominent attempts to fill Kripke’s cognitive gap, most notably Evans’s discriminating conception, Recanati’s mental files, and Dickie’s perceptual-attention framework. These accounts correctly recognize that reference requires a non-descriptive cognitive link to an object (what Recanati calls an 'Epistemically Rewarding' relation). The present account builds on this insight but shifts the focus: rather than treating reference-fixing primarily as a matter of internal mental representations or private files, it frames it as a publicly usable, dual-mode linguistic operation ($x^o / x^c$). The prior capacity to single out a particular in an encounter is what I will call _local rigidity_.
...
The proposal pursued in the next chapter is that a primitive tracking capacity — _local rigidity_ — can be used to separate those two notions and to supply the missing cognitive layer. Chapter 4 introduces the analytical vocabulary in which that proposal is formulated: the object-mode/concept-mode distinction $x^o / x^c$, the grounding operator $\mathcal{R}$, and the apparatus needed to articulate the necessary a posteriori and de re modality in PRU terms.